...
function getNextFriday(date) {
  ...
  const nextFriday = new Date(
    date.getTime() + daysUntilFriday * 1000 * 60 * 60 * 24
  );
  return nextFriday;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysPassed = Math.floor(
    (date.getTime() - firstDayOfYear.getTime() + daysOffset * 86400000) /
      86400000
  );
...
function getNextFridayThe13th(date) {
  ...
  let month = date.getDate() >= 13 ? date.getMonth() + 1 : date.getMonth();
  let friday13;
  do {
